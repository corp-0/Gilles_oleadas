...
MAX_ENEMY_PER_WAVE = 50; //How many enemies can be in the map at once.
...
TIME_BETWEEN_SPAWNS = 10; //Time to make the script wait until next spawn
...
if(_spawnMarkers isEqualTo []) exitWith {
    diag_log "GO: Couldn't find any spawn markers";
    };
if(_attackMarkers isEqualTo []) exitWith {
    diag_log "GO: Couldn't find any attack markers";;
    };
